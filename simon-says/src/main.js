...
function resultHandler(isRight) {
  if (isRight) {
    ...
    if (gameParams.isMistakeOne) {
      newGameInit();
      elementList.roundPanel.textContent = `Game over! Select difficulty setting and press "Start"`;
    } else {
      elementList.virtualKB.classList.add('inactive');
      gameParams.isMistakeOne = true;
      elementList.roundPanel.textContent = `${elementList.roundPanel.textContent} \n Watch out! One more mistake and you'll have to start over! Try to repeat the sequence`;
    }
  }
}
